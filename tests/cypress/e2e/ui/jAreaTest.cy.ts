import { addNode, createSite, deleteSite, enableModule, getNodeAcl, getNodeByPath, waitUntilJournalSync } from "@jahia/cypress"
...
describe('jArea helper test', () => {
    before('Create site and content', () => {
        ...
        cy.get('div[class="article"]').find('div[type="placeholder"]').should('be.visible').then(() => {
            addNode({
    ...
    it('Allowed types area', () => {
        cy.iframe('#page-builder-frame-1').within(() => {
            cy.get('div[data-testid="allowedTypesArea"]').find('div[type="placeholder"]').then(buttons => {
                ...
                cy.get(selector).find('button[data-sel-role!="jnt:event"][data-sel-role!="jnt:bigText"]').should('not.exist');
            })
        })
    })
