        ...
        it('Check the transformed module headers', () => {
            cy.executeGroovy('groovy/getBundleHeaders.groovy', {BUNDLE_SYMBOLIC_NAME: 'jahia-npm-module-exampl_e', BUNDLE_VERSION: '1.0.0'})
                .then(result => {
                    ...
                    expect(result).to.contain('Bundle-Name: @jahia/npm-module-examplée (npm module)');
                    expect(result).to.contain('Bundle-SymbolicName: jahia-npm-module-exampl_e');
                    expect(result).to.contain('Bundle-Vendor: Jahia Solutions Group SA');
                    expect(result).to.contain('Bundle-Version: 1.0.0');
                    expect(result).to.contain('Jahia-Depends: calendar,event');
                    ...
                    expect(result).to.contain('Jahia-Static-Resources: /css,/icons,/images,/img,/javascript');
                });
        });
    }
);
